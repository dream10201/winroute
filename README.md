...
- 然后把期望的路由“对账”进系统路由表：
  ...
  - 同样在这种情况下，若公司网卡的接口跃点数（metric）不高于路由器网卡，会把公司网卡临时调到 9999，让 Windows 优先使用路由器网卡的 DNS 服务器。该修改只写入活动存储，重启或网卡重连即恢复，程序退出或条件不再满足时也会还原。
...
- `force_default_to_router`：两网都在线时是否把默认路由和 DNS 抢到路由器，默认 `true`。
